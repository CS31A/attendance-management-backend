    ...
    ILogger<FingerprintController> logger) : ControllerBase
{
    ...
    /// <response code="200">Fingerprint registered successfully</response>
    /// <response code="400">Invalid input data or validation error</response>
    /// <response code="401">User not authenticated</response>
    /// <response code="403">User not authorized (Admin/Instructor only)</response>
    ...
    public async Task<ActionResult<FingerprintRegistrationResponseDto>> RegisterFingerprint([FromBody] RegisterFingerprint request)
    {
        logger.LogInformation("Fingerprint registration request for StudentId: {StudentId}", request.StudentId);

        if (!ModelState.IsValid)
        {
            logger.LogWarning("Fingerprint registration failed due to invalid model state for StudentId: {StudentId}", request.StudentId);
        ...
        try
        {
            var response = await fingerprintService.RegisterFingerprintAsync(request, User);
            logger.LogInformation("Fingerprint registered successfully for StudentId: {StudentId}", request.StudentId);
            return Ok(response);
        }
        catch (EntityNotFoundException<int> ex)
        {
            logger.LogWarning(ex, "Student not found for fingerprint registration");
        ...
        {
            logger.LogWarning(ex, "Validation error during fingerprint registration");
            return BadRequest(new FingerprintRegistrationResponseDto
            {
                Success = false,
                Message = ex.Message
        ...
        {
            logger.LogWarning(ex, "Unauthorized fingerprint registration attempt");
    ...
    [ProducesResponseType(typeof(FingerprintRegistrationResponseDto), StatusCodes.Status403Forbidden)]
    [ProducesResponseType(typeof(FingerprintRegistrationResponseDto), StatusCodes.Status404NotFound)]
    public async Task<ActionResult<FingerprintRegistrationResponseDto>> UpdateFingerprint(
        int fingerprintId,
        [FromBody] RegisterFingerprint request)
    {
        logger.LogInformation("Fingerprint update request for ID: {FingerprintId}", fingerprintId);
        ...
        try
        {
            var response = await fingerprintService.UpdateFingerprintAsync(fingerprintId, request, User);
            logger.LogInformation("Fingerprint updated successfully for ID: {FingerprintId}", fingerprintId);
            return Ok(response);
        }
        catch (EntityNotFoundException<int> ex)
        {
            logger.LogWarning(ex, "Fingerprint not found for update");
            return NotFound(new FingerprintRegistrationResponseDto
            {
                Success = false,
                Message = ex.Message
        ...
        {
            logger.LogWarning(ex, "Unauthorized fingerprint update attempt");
            return StatusCode(StatusCodes.Status403Forbidden, new FingerprintRegistrationResponseDto
            {
                Success = false,
                Message = ex.Message
    ...
    /// This endpoint is typically called by biometric devices.
    /// </summary>
    /// <param name="request">The fingerprint scan request.</param>
    /// <returns>Scan result with attendance status.</returns>
    /// <response code="200">Scan processed successfully (attendance may or may not be marked)</response>
    /// <response code="400">Invalid input data</response>
    [HttpPost("scan")]
    [AllowAnonymous] // Biometric devices may not have JWT tokens
    [ProducesResponseType(typeof(FingerprintScanResponseDto), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(FingerprintScanResponseDto), StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<FingerprintScanResponseDto>> ScanFingerprint([FromBody] ScanFingerprint request)
    {
        logger.LogInformation("Fingerprint scan request from device: {DeviceId}", request.DeviceId);

        if (!ModelState.IsValid)
        {
            logger.LogWarning("Fingerprint scan failed due to invalid model state");
            return BadRequest(new FingerprintScanResponseDto
            {
                Success = false,
                Message = "Invalid request data"
        ...
        try
        {
            var response = await fingerprintService.ScanFingerprintAsync(request);
            logger.LogInformation("Fingerprint scan processed for device: {DeviceId}, Success: {Success}",
                request.DeviceId, response.Success);
            return Ok(response);
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Error processing fingerprint scan from device {DeviceId}", request.DeviceId);
            return Ok(new FingerprintScanResponseDto
            {
                Success = false,
                Message = "An error occurred while processing the fingerprint scan"
            });
        }
    }
    ...
    /// Used for pre-scan validation or enrollment verification.
    /// </summary>
    /// <param name="request">The validation request with template data.</param>
    /// <returns>Validation result with student information if match found.</returns>
    /// <response code="200">Validation processed successfully</response>
    /// <response code="400">Invalid input data</response>
    [HttpPost("validate")]
    [AllowAnonymous]
    [ProducesResponseType(typeof(FingerprintScanResponseDto), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(FingerprintScanResponseDto), StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<FingerprintScanResponseDto>> ValidateFingerprint([FromBody] ScanFingerprint request)
    {
        logger.LogInformation("Fingerprint validation request from device: {DeviceId}", request.DeviceId);
        ...
        try
        {
            var response = await fingerprintService.ValidateFingerprintAsync(request.TemplateData);
            return Ok(response);
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Error validating fingerprint from device {DeviceId}", request.DeviceId);
            return Ok(new FingerprintScanResponseDto
            {
                Success = false,
                Message = "An error occurred while validating the fingerprint"
            });
        }
    }
