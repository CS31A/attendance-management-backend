...
public sealed class UuidRouteIntegrationTests
{
    ...
    public async Task GetCourseByUuid_ReturnsSameCourseAsLegacyIntRoute()
    {
        ...
        var uuidResponse = await host.Client.GetAsync($"/api/course/uuid/{expected.Uuid}");
    ...
    public async Task GetSubjectByUuid_ReturnsSameSubjectAsLegacyIntRoute()
    {
        ...
        var uuidResponse = await host.Client.GetAsync($"/api/subjects/uuid/{expected.Uuid}");
    ...
    public async Task GetSectionByUuid_ReturnsSameSectionAsLegacyIntRoute_WithCourseUuidSidecar()
    {
        ...
        var uuidResponse = await host.Client.GetAsync($"/api/sections/uuid/{expected.Uuid}");
        ...
        Assert.Equal(expected.Id, uuidPayload.Id);
        Assert.Equal(expected.Uuid, uuidPayload.Uuid);
        Assert.Equal(expected.CourseId, uuidPayload.CourseId);
        Assert.Equal(expected.Course!.Uuid, uuidPayload.CourseUuid);
        Assert.Equal(expected.Name, uuidPayload.Name);
    }
    ...
    public async Task GetClassroomByUuid_ReturnsSameClassroomAsLegacyIntRoute()
    {
        ...
        var uuidResponse = await host.Client.GetAsync($"/api/classrooms/uuid/{expected.Uuid}");
    ...
    public async Task GetScheduleByUuid_ReturnsSameScheduleAsLegacyIntRoute_WithNestedUuidSidecars()
    {
        ...
        var uuidResponse = await host.Client.GetAsync($"/api/schedules/uuid/{expected.Uuid}");
        ...
        Assert.Equal(expected.Subject.Uuid, uuidPayload.Subject.Uuid);
        Assert.Equal(expected.Classroom.Id, uuidPayload.Classroom.Id);
        Assert.Equal(expected.Classroom.Uuid, uuidPayload.Classroom.Uuid);
        Assert.Equal(expected.Section.Id, uuidPayload.Section.Id);
        Assert.Equal(expected.Section.Uuid, uuidPayload.Section.Uuid);
        Assert.Equal(expected.Section.CourseId, uuidPayload.Section.CourseId);
        Assert.Equal(expected.Section.Course!.Uuid, uuidPayload.Section.CourseUuid);
        Assert.Equal(expected.Instructor.Id, uuidPayload.Instructor.Id);
        Assert.Equal(expected.Instructor.Uuid, uuidPayload.Instructor.Uuid);
    }
    ...
    public async Task GetSessionByUuid_ReturnsSameSessionAsLegacyIntRoute_WithUuidSidecars()
    {
        ...
        var uuidResponse = await host.Client.GetAsync($"/api/sessions/uuid/{expected.Uuid}");
        ...
        Assert.Equal(expected.Id, uuidPayload.Id);
        Assert.Equal(expected.Uuid, uuidPayload.Uuid);
        Assert.Equal(expected.ScheduleId, uuidPayload.ScheduleId);
        Assert.Equal(expected.Schedule!.Uuid, uuidPayload.ScheduleUuid);
        Assert.Equal(expected.ActualRoomId, uuidPayload.ActualRoomId);
        Assert.Equal(expected.ActualRoom?.Uuid, uuidPayload.ActualRoomUuid);
    }
    ...
    public async Task GetStudentEnrollmentsByStudentUuid_ReturnsSameEnrollmentSetAsLegacyIntRoute_WithUuidSidecars()
    {
        ...
        var uuidResponse = await host.Client.GetAsync($"/api/studentenrollment/student/uuid/{expected.Student!.Uuid}");
        ...
        Assert.Equal(legacyPayload.StudentId, uuidPayload.StudentId);
        Assert.Equal(expected.StudentId, uuidPayload.StudentId);
        Assert.Equal(expected.Student.Uuid, uuidPayload.StudentUuid);
        Assert.Equal(legacyEnrollment.EnrollmentId, uuidEnrollment.EnrollmentId);
        Assert.Equal(expected.Uuid, uuidEnrollment.EnrollmentUuid);
        Assert.Equal(expected.SectionId, uuidEnrollment.SectionId);
        Assert.Equal(expected.Section!.Uuid, uuidEnrollment.SectionUuid);
        Assert.Equal(expected.SubjectId, uuidEnrollment.SubjectId);
        Assert.Equal(expected.Subject!.Uuid, uuidEnrollment.SubjectUuid);
    }
    ...
    public async Task GetAttendanceByUuid_ReturnsSameAttendanceAsLegacyIntRoute_WithUuidSidecars()
    {
        ...
        var uuidResponse = await host.Client.GetAsync($"/api/attendance/uuid/{expected.Uuid}");
        ...
        Assert.Equal(expected.StudentId, uuidPayload.StudentId);
        Assert.Equal(expected.Student!.Uuid, uuidPayload.StudentUuid);
        Assert.Equal(expected.SessionId, uuidPayload.SessionId);
        Assert.Equal(expected.Session!.Uuid, uuidPayload.SessionUuid);
        Assert.Equal(expected.Session.ScheduleId, uuidPayload.ScheduleId);
        Assert.NotEqual(Guid.Empty, uuidPayload.ScheduleUuid);
    }
    ...
    public async Task GetSessionAttendanceByUuid_ReturnsSameOverviewAsLegacyIntRoute_WithSessionAndAttendanceUuids()
    {
        ...
        var uuidResponse = await host.Client.GetAsync($"/api/attendance/session/uuid/{expectedSession.Uuid}");
        ...
        Assert.Equal(expectedSession.Uuid, uuidPayload.SessionUuid);
        Assert.Equal(expectedSession.ScheduleId, uuidPayload.ScheduleId);
        Assert.Equal(expectedSession.Schedule.Uuid, uuidPayload.ScheduleUuid);
        Assert.Equal(legacyPayload.AttendanceRecords[0].AttendanceRecordId, uuidPayload.AttendanceRecords[0].AttendanceRecordId);
        Assert.NotEqual(Guid.Empty, uuidPayload.AttendanceRecords[0].AttendanceRecordUuid ?? Guid.Empty);
        Assert.NotEqual(Guid.Empty, uuidPayload.AttendanceRecords[0].StudentUuid ?? Guid.Empty);
    }
    ...
    public async Task GetQrCodeByUuid_ReturnsSameQrCodeAsLegacyIntRoute_WithUuidSidecars()
    {
        ...
        var uuidResponse = await host.Client.GetAsync($"/api/qrcode/uuid/{expected.Uuid}");
        ...
        Assert.Equal(expected.Session!.Uuid, uuidPayload.SessionUuid);
        Assert.Equal(expected.Session.ScheduleId, uuidPayload.ScheduleId);
        Assert.NotEqual(Guid.Empty, uuidPayload.ScheduleUuid ?? Guid.Empty);
